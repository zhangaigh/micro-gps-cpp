...
for dimensionality_idx = 1 : length(dimensionality_array)
  ...
  for ds_idx = 1 : length(dataset_info)
    ...
    for vw_idx = 1 : length(visual_words_array)
      vw_name = visual_words_array{vw_idx};
      fprintf('%s\n', vw_name);
      success_flag_cat = [];

      for ts_idx = 1 : length(curr_dataset_info.test_sequences)
        testset_name = curr_dataset_info.test_sequences{ts_idx};
        output_folder = sprintf('output-vw-%s-%s-%s-db%d-dim%d', dataset_name, vw_name, testset_name, db_sample_size, dimensionality);
...
if 0
  if ismac
    fid = fopen('../bin_mac/script_visual_words.sh', 'w');
  else
    fid = fopen('../bin/script_visual_words.sh', 'w');
  end
