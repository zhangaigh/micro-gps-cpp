...
Mat calibratedCameraMatrix, calibratedDistCoeffs;
Mat map1, map2;
    ...
    case CIRCLES_GRID:
      for (int i = 0; i < boardSize.height; i++)
        for (int j = 0; j < boardSize.width; j++)
          corners.push_back(
              Point3f(float(j * SQUARE_SIZE_X), float(i * SQUARE_SIZE_Y), 0));
      break;
...
bool runCalibration(vector<vector<Point2f> > imagePoints, Size imageSize,
  ...
  double rms = calibrateCamera(
      objectPoints, imagePoints, imageSize, cameraMatrix, distCoeffs, rvecs,
      tvecs, flags | CV_CALIB_ZERO_TANGENT_DIST | CV_CALIB_FIX_K3 |
...
bool runAndSave(const string& outputFilename,
  ...
  if (ok)
    saveCameraParams(
        outputFilename, imageSize, boardSize, squareSize, aspectRatio, flags,
...
void calibrateImageSequence(std::vector<char*> imageList, const char* outputFilename) {
  ...
  for (int i = 0; i < imageList.size(); i++) {
    ...
    view = imread(imageList[i], CV_LOAD_IMAGE_GRAYSCALE);
    printf("imageList[%d] = %s\n", i, imageList[i]);
    imageSize = view.size();
  ...
  if (imagePoints.size() > 0) {
    runAndSave(string(outputFilename), imagePoints, imageSize, boardSize, pattern,
...
void readCalibrationParameters(const char* calibration_file) {
  FileStorage fs(string(calibration_file), FileStorage::READ);
  Size imageSize;
  
  fs["camera_matrix"] >> calibratedCameraMatrix;
  fs["distortion_coefficients"] >> calibratedDistCoeffs;
  fs["image_width"] >> imageSize.width;
  fs["image_height"] >> imageSize.height;
  fs.release();
  ...
  initUndistortRectifyMap(calibratedCameraMatrix, calibratedDistCoeffs, Mat(),
                          calibratedCameraMatrix,
                          imageSize, CV_16SC2, map1, map2);
}

void undistortImage(cv::Mat& image, cv::Mat& undistorted_image) {
  // undistort(image, undistorted_image, calibratedCameraMatrix, calibratedDistCoeffs);  
  remap(image, undistorted_image, map1, map2, INTER_LINEAR);
}
