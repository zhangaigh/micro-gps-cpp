...
#if ON_MAC
DEFINE_string(dataset_root,    "/Users/lgzhang/Documents/DATA/micro_gps_packed", "dataset root");
#endif
...
int main (int argc, char** argv) {
  ...
  int num_frames = extracting_test ? dataset->getTestSequenceSize() : dataset->getDatabaseSize();
  ...
  for (int i = 0; i < num_frames; i++ ){
    ...
    if (extracting_test) {
      ...
      dataset->getTestImagePrecomputedFeatures(i, precomputed_sift_path, (char*)"sift");
      ...
      dataset->getDatabaseImagePrecomputedFeatures(i, precomputed_sift_path, (char*)"sift");
      current_test_frame = new MicroGPS::Image(dataset->getDatabaseImagePath(i),
                                               precomputed_feat_path,
                                               precomputed_sift_path);
    }
    ...
    size_t patch_cnt = 0;
    for (size_t f_idx = 0; f_idx < current_test_frame->getNumLocalFeatures(); f_idx++) {
      ...
      if (f->x > margin_thresh && f->x < w-1-margin_thresh &&
        ...
        // crop 
        MicroGPS::Image* patch = MicroGPS::ImageFunc::cropPatch(current_test_frame, f->x, f->y, f->angle, 
                                                               FLAGS_patch_size, FLAGS_patch_size);
        
        fwrite(patch->data(), sizeof(uchar), patch->width()*patch->height(), fp);
        patch_cnt++;
    ...
    printf("cropped %lu / %lu patches\n", patch_cnt, current_test_frame->getNumLocalFeatures());
...
//   lena.loadImage();
//   printf("lena size = %ld x %ld x %ld\n", lena.width(), lena.height(), lena.channels());
//   lena.bgr2gray();

//   // crop 
//   MicroGPS::Image* patch = MicroGPS::ImageFunc::cropPatch(&lena, 256.0f, 256.0f, 45.0f / 180.0f * 3.14f, 
//                                                           64, 64);
